'''
Based on the version of 1116.
Modified on 30 Nov, 2018. Some unnecessary parts are deleted.
'''
# coding=utf-8
...
def MatMulss(A1, A2, B1, B2):
	...
	C1 = np.zeros((dim1, dim2))
	C2 = np.zeros((dim1, dim2))
	toff = 0
	ton = 0
	for i in range(dim1):
		for j in range(dim2):
			C1[i][j], C2[i][j], off0, on0 = InnerProductss(A1[i,:], A2[i,:], B1[:,j], B2[:,j])
			toff += off0
			ton += on0
	return C1, C2, toff, ton
